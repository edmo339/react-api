...
const AudioPlayer = () => {
   ...
   const [currentTime, setCurrentTime] = useState(0);
    ...
    const animationRef = useRef(); // refernce to animation
    ...
    } , []);//, [audioPlayer?.current.metadata, audioPlayer?.current?.readyState]);
  ...
  return (
    <div className= {styles.audioPlayer}>
        <audio ref={audioPlayer} src='https://' preload='metadata'></audio>
        ...
        {/* current time */}
        <div className={styles.currentTime}>{calculatedTime(currentTime)}</div>
        ...
        {/* duration */}
        <div className={styles.duration}>{(duration && isNaN(duration)) && calculatedTime(duration)}</div>
    </div>
  )
}
